...
import org.apache.hadoop.hbase.regionserver.Index.IndexUtil;
...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner
   ...
   */
  public StoreScanner(Store store, ScanInfo scanInfo, Scan scan,
                      List<? extends KeyValueScanner> scanners, boolean isIndex,
                      long smallestReadPoint, long earliestPutTs,
                      byte[] dropDeletesFromRow, byte[] dropDeletesToRow) throws IOException {
    this(store, scanInfo, scan, scanners, ScanType.INDEX_DELETES, smallestReadPoint,
            earliestPutTs, dropDeletesFromRow, dropDeletesToRow);
    this.scanType = ScanType.INDEX_DELETES;
    LOG.error("is index repair scanner: " + isIndex);
  }
...
//      LOG.error(qcode);
      switch(qcode) {
        ...
        case SEEK_NEXT_COL:
          if (this.scanType == ScanType.INDEX_DELETES) {
            byte[] dataKeyTodels = cell.getRow();
            byte[] columnFamily = cell.getFamily();
            byte[] columnName = cell.getQualifier();
            byte[] value = cell.getValue();
            long ts = cell.getTimestamp();

            if (Bytes.equals(columnFamily, IndexUtil.INDEX_CF) && Bytes.equals(columnName, IndexUtil.INDEX_COL)) {
              String line = Bytes.toString(dataKeyTodels) + "," + Bytes.toString(columnFamily) + "," +
                      Bytes.toString(columnName) + "," + Bytes.toString(value) + "," + ts;
//            LOG.error(line);
              IndexUtil.sendToAUQ(line, this.store.getRegionInfo().getTable().toString());
            }
