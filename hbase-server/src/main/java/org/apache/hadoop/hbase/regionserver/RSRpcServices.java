...
import org.apache.hadoop.hbase.protobuf.generated.AdminProtos.FlushIndexRegionRequest;
import org.apache.hadoop.hbase.protobuf.generated.AdminProtos.FlushIndexRegionResponse;
...
public class RSRpcServices implements HBaseRPCErrorHandler,
  ...
  @Override
  @QosPriority(priority=HConstants.ADMIN_QOS)
  public FlushIndexRegionResponse flushIndexRegion(final RpcController controller,
                                         final FlushIndexRegionRequest request) throws ServiceException {
    try {
      checkOpen();
      requestCount.increment();
      Region region = getRegion(request.getRegion());
      ...
      FlushIndexRegionResponse.Builder builder = FlushIndexRegionResponse.newBuilder();
      if (shouldFlush) {
        boolean writeFlushWalMarker =  request.hasWriteFlushWalMarker() ?
                request.getWriteFlushWalMarker() : false;
        long startTime = EnvironmentEdgeManager.currentTime();
        // Go behind the curtain so we can manage writing of the flush WAL marker
        HRegion.FlushResultImpl flushResult = (HRegion.FlushResultImpl)
                ((HRegion)region).flushIndexcache(true, writeFlushWalMarker);
        if (flushResult.isFlushSucceeded()) {
          long endTime = EnvironmentEdgeManager.currentTime();
          regionServer.metricsRegionServer.updateFlushTime(endTime - startTime);
        }
