...
public class IndexKeyCompactionPolicy extends ExploringCompactionPolicy {
    ...
    @Override
    public List<StoreFile> applyCompactionPolicy(List<StoreFile> candidates, boolean mightBeStuck, boolean isOffpeak, int minFiles, int maxFiles) {
...
//        return new ArrayList<>(bestSelection);
    ...
    private boolean isOverlap(StoreFile firstSF, StoreFile secondSF) {
        byte[] start1 = firstSF.getFirstKey();
        byte[] end1 = firstSF.getLastKey();
        byte[] start2 = secondSF.getFirstKey();
        byte[] end2 = secondSF.getLastKey();

        return !(Bytes.compareTo(end1, start2) <= 0 || Bytes.compareTo(start1, end2) >= 0);
