...
public class StripeStoreFlusher extends StoreFlusher {
  ...
  public List<Path> flushIndexSnapshot(MemStoreSnapshot snapshot, long cacheFlushSeqNum,
                                  MonitoredTask status, ThroughputController throughputController,
                                  FlushLifeCycleTracker tracker) throws IOException {
    List<Path> result = new ArrayList<>();
    int cellsCount = snapshot.getCellsCount();
    ...
    long smallestReadPoint = store.getSmallestReadPoint();
    InternalScanner scanner = createIndexScanner(snapshot.getScanners(), smallestReadPoint, tracker);
