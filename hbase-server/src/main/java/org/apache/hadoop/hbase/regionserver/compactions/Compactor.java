...
public abstract class Compactor {
  ...
  protected InternalScanner createScanner(Store store, List<StoreFileScanner> scanners, boolean isIndex,
                                        long smallestReadPoint, long earliestPutTs,
                                          byte[] dropDeletesFromRow, byte[] dropDeletesToRow) throws IOException {
    Scan scan = new Scan();
    scan.setMaxVersions(store.getFamily().getMaxVersions());
    return new StoreScanner(store, store.getScanInfo(), scan, scanners, isIndex, smallestReadPoint,
            earliestPutTs, dropDeletesFromRow, dropDeletesToRow);
  }
}
