...
public class HBaseAdmin implements Admin {
   ...
   */
  @Override
  public void flushIndex(final TableName tableName) throws IOException {
    checkTableExists(tableName);
    if (isTableDisabled(tableName)) {
      LOG.info("Table is disabled: " + tableName.getNameAsString());
      return;
    }
    execProcedure("flush-index-table-proc", tableName.getNameAsString(),
            new HashMap<String, String>());
  }

  @Override
  public void flushIndexRegion(final byte[] regionName) throws IOException {
    Pair<HRegionInfo, ServerName> regionServerPair = getRegion(regionName);
    if (regionServerPair == null) {
      throw new IllegalArgumentException("Unknown regionname: " + Bytes.toStringBinary(regionName));
    }
    ...
    flushIndex(regionServerPair.getSecond(), regionServerPair.getFirst());
  }
   ...
   * New
   */
  public void flushIndex(final String tableNameOrRegionName)
          throws IOException, InterruptedException {
    flushIndex(Bytes.toBytes(tableNameOrRegionName));
  }

  public void flushIndex(final byte[] tableNameOrRegionName)
          throws IOException, InterruptedException {
    try {
      flushIndexRegion(tableNameOrRegionName);
    } catch (IllegalArgumentException e) {
      // Unknown region.  Try table.
      flushIndex(TableName.valueOf(tableNameOrRegionName));
    }
  }

  private void flushIndex(final ServerName sn, final HRegionInfo hri)
          throws IOException {
    PayloadCarryingRpcController controller = rpcControllerFactory.newController();
    AdminService.BlockingInterface admin = this.connection.getAdmin(sn);
    AdminProtos.FlushIndexRegionRequest request =
            RequestConverter.buildFlushIndexRegionRequest(hri.getRegionName());
    try {
      admin.flushIndexRegion(controller, request);
    } catch (ServiceException se) {
      throw ProtobufUtil.getRemoteException(se);
    }
  }
